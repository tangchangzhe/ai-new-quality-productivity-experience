...
export async function getVoteDistribution({ includeIdeaId = null } = {}) {
  const params = [];
  let includeCurrentClause = "";
  if (includeIdeaId) {
    includeCurrentClause = " OR v.idea_id = ?";
    params.push(includeIdeaId);
  }

  const [rows] = await getPool().execute(
    `SELECT v.voted_model, COUNT(*) AS count
     FROM votes v
     INNER JOIN ideas i ON i.id = v.idea_id
     WHERE i.seeded = 1 OR i.is_complete = 1${includeCurrentClause}
     GROUP BY v.voted_model`,
    params
  );
